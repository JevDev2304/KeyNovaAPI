...
async def furniture(Habitacion_idHabitacion: int,estado : str ,nombre: str, descripcion = None, image: UploadFile =File(...)):
    check_state(estado)
    img_dir = await upload_img(ABSOLUTE_IMG_DIR, image)
    dict_furniture ={"Habitacion_idHabitacion": Habitacion_idHabitacion, "estado":estado,"nombre": nombre ,"imagen":img_dir, "descripcion" : descripcion}
...
async def update_strings(id:int , estado : str,nombre :str, descripcion : str):
    ...
    final_furniture = furniture_schema(dbConnection.obtener_mueble_por_id(id))
...
def check_state(state : str):
    ...
    if state in states:
        ...
        return HTTPException(status_code= status.HTTP_405_METHOD_NOT_ALLOWED, detail = "You cannot post a furniture with this state ,please only use these states( pesimo, malo , regular , bueno , excelente)  ")